...
- `MergingPress` ([source](kvpress/presses/merging_press.py)): A prefill press that wraps any `ScorerPress` or `KVzipPress` (incl. `KVgradPress`, `RestoreKVPress`) and folds each evicted token's value into its most cosine-similar surviving neighbor (keys preserved by default). 🤖 automated agent contribution
