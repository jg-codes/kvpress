...
"""
MergingPress(DMSPress(KVzapPress)) evaluation — can merge-on-evict beat SOTA?

Evaluates MergingPress wrapping DMSPress at thresholds -4 and -3 against bare
DMSPress(KVzapPress) baselines on RULER-4096 / Qwen3-8B.

Speed optimization: parallel execution via starmap — all configs run simultaneously
on separate GPUs. At f=0.1 this completes in ~7 min wall-clock.

Usage:
    modal run --detach evaluation/modal_dms_merging.py                    # f=0.1 quick
    modal run --detach evaluation/modal_dms_merging.py --fraction 1.0     # full leaderboard
"""
...
image = (
    ...
    # Install kvpress from dev branch with DMSPress+MergingPress support
    .run_commands(
...
app = modal.App("kvpress-dms-merging", image=image)
results_vol = modal.Volume.from_name("kvpress-dms-merging-results", create_if_missing=True)
...
# ── Evaluation configs ──────────────────────────────────────────────────
#
# Theory-driven parameter choices (perturbation bound analysis):
#
#   ‖dO_merge‖ <= a_{t,i} * ‖v_i‖ * (1-w)/(1+w)
#
#   DMSPress evicts tokens with low importance (small a_{t,i}), so merging
#   those tokens has low error even at moderate cosine similarity w.
#
#   Optimal defaults:
#     similarity_threshold=0.0  → merge ALL (even low-w reduces error)
#     value_norm_weighting=True → weight by ‖v‖ (high-value merges first)
#     merge_keys=False          → preserve RoPE (-2.5 pp empirically)
#     max_merge_per_token=0     → unlimited (safe at <=60% eviction)
#
#   merge_fraction=0.75 ablation: tests whether gating out weakest 25%
#   of merges helps. Theory says no; empirical may differ.
#
CONFIGS = [
    # ── Baselines ──
    {"name": "no_press", "threshold": None, "merge_params": None},
    {"name": "dms_kvzap__t-4", "threshold": -4, "merge_params": None},
    {"name": "dms_kvzap__t-3", "threshold": -3, "merge_params": None},
    # ── MergingPress(DMSPress) — default (theory-optimal) ──
    {"name": "m_dms_kvzap__t-4__default", "threshold": -4, "merge_params": {}},
    {"name": "m_dms_kvzap__t-3__default", "threshold": -3, "merge_params": {}},
    # ── Ablation: selective merging (gate out weakest 25%) ──
    {"name": "m_dms_kvzap__t-4__mf0.75", "threshold": -4, "merge_params": {"merge_fraction": 0.75}},
    {"name": "m_dms_kvzap__t-3__mf0.75", "threshold": -3, "merge_params": {"merge_fraction": 0.75}},
]
...
@app.function(
    gpu="A100",
    timeout=25200,  # 7h for full dataset
    memory=65536,
    scaledown_window=2,
    secrets=_secrets,
...
def main(fraction: float = 0.10):
    """Launch MergingPress(DMSPress) evaluation on Modal — all configs in parallel."""
    n_baselines = sum(1 for c in CONFIGS if c["merge_params"] is None)
    n_merging = len(CONFIGS) - n_baselines

    print(f"\n{'='*90}")
    print("MergingPress(DMSPress(KVzapPress)) — SOTA Challenge")
    ...
    # ── Summary table ────────────────────────────────────────────────
    print(f"\n{'='*90}")
    print(f"{'Name':<42} {'Score':>6} {'CR':>7} {'Infer':>7} {'Load':>6} {'Threshold':>5}")
    print(f"{'-'*90}")
    ...
    out_dir = pathlib.Path("evaluation/results_dms_merging")
